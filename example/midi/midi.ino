...
/*
  Number of miliseconds to play the note.
*/
int duration = 500;

/*
  Combine the command for turning a note on with the channel number to arrive at the full
  MIDI command. We will specify which note to play later. This is just for turning any
  old note on.
*/
...
/*
  ...
  MIDI command. We will specify which note to turn off later. This is just for turning any
  old note off.
*/
...
/*
  ...
  arrive at the full MIDI command. We will specify what we wish to change about the channel later.
*/
byte onChannelOneChangeMode = MIDI_CONTROL_MODE_CHANGE | MIDI_CHANNEL_1;

/*
  Value for the volume. Ranges from 0x00 ( 0 in decimal ) to 0x7f ( 127 in decimal ).
*/
byte volume = 0x7f;
...
void loop() {
  // TODO Determine if once is enough or this must be changed periodically.
